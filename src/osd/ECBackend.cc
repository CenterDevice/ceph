...
void ECBackend::start_write(Op *op) {
  ...
  // we may need to dup some transactions if the same shard appears
  // multiple times in actingbackfill.  others won't be used at all
  // and need to get freed.
  vector<ObjectStore::Transaction*> transvec(actingbackfill.size());
  vector<bool> trans_used(trans.size(), false);
  int j = 0;
  for (set<pg_shard_t>::const_iterator i = actingbackfill.begin();
	...
	trans.find(i->shard);
      ...
      if (!trans_used[i->shard]) {
	// first instance of this shard
	transvec[j] = iter->second;
	trans_used[i->shard] = true;
	dout(20) << __func__ << " first " << i->shard << " gets " << transvec[j] << dendl;
      } else {
	// make a copy of the txn
	transvec[j] = new ObjectStore::Transaction;
	*transvec[j] = *iter->second;
	dout(20) << __func__ << " dup   " << i->shard << " gets " << transvec[j] << dendl;
      }
    } else {
      dout(20) << __func__ << " null " << i->shard << dendl;
      transvec[j] = NULL;
    }
  }
  for (map<shard_id_t,ObjectStore::Transaction*>::iterator p = trans.begin();
       p != trans.end(); ++p) {
    if (!trans_used[p->first]) {
      dout(20) << __func__ << " deleting " << p->first << " " << p->second << dendl;
      delete p->second;
    }
  }
  ...
  for (set<pg_shard_t>::const_iterator i = actingbackfill.begin();
    ...
    ObjectStore::Transaction *t = transvec[j];
    pg_stat_t stats =
      t ?
      get_info().stats :
